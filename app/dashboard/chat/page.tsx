...
export default function SangamChat() {
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      ...
      // Mark all messages in this chat as read (where I'm NOT the sender)
    ...
    // REALTIME MAGIC - Listen to ALL messages
    const channel = supabase
      ...
      .on('postgres_changes', 
        ...
        async (payload) => {
          ...
          // Case A: Message is for the ACTIVE chat
          if (newMsg.connection_id === activeChat) {
            setMessages(prev => [...prev, newMsg]);
            
            // Mark as read immediately if I'm not the sender
          ...
          // Case B: Message is from someone else (Update the badge!)
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-2">
          ...
          {conversations.map((c) => (
            <button
              key={c.connection_id}
              onClick={() => {
                setActiveChat(c.connection_id);
                // Clear the unread badge for this user
                setUnreadCounts(prev => ({ ...prev, [c.partner.id]: 0 }));
              }}
            ...
            >
              ...
              {unreadCounts[c.partner.id] > 0 && (
                <span className="bg-haldi-500 text-black text-[10px] font-bold h-5 w-5 rounded-full flex items-center justify-center shadow-md flex-shrink-0">
      ...
      <div className="flex-1 flex flex-col bg-stone-950 relative">
        {!activeChat ? (
          ...
          <>
            ...
            {/* Messages */}
            <div ref={messagesContainerRef} className="flex-1 overflow-y-auto p-6 space-y-4 bg-stone-950">
               {messages.length === 0 && (
